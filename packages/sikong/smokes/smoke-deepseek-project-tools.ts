 ...
 *
 * The smoke creates a marker file under a unique .sikong-smoke-* directory,
...
const workspaceDir = flag("--dir") ?? join(await mkTempDir("sikong-deepseek-workspace-"), ".sikong");
const projectRoot = flag("--project-root") ?? (await mkTempDir("sikong-deepseek-project-"));
const marker = `ds-tool-smoke-${randomUUID().slice(0, 8)}`;
const smokeDir = `.sikong-smoke-${marker}`;
