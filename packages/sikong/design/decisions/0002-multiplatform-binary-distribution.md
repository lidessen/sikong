...
`sikong` ships as a Bun single-file executable (`bun build --compile`). The
...
sikong` fails with `EBADPLATFORM` on every other platform — effectively
macOS-arm64-only.
...
1. **One package per platform** — `sikong-<key>` (`darwin-arm64`, `darwin-x64`,
...
2. **`sikong` is a thin cross-platform launcher** — no `os`/`cpu` lock, no
   binary. `bin/sikong` is a Node ESM shim that resolves
   `sikong-<key>/bin/<exe>` (via `import.meta.resolve`, with filesystem-sibling
   fallback for older Node and a `dist/` fallback for local dev) and execs it.
3. **Platform packages are optional dependencies** of `sikong`, pinned to the
...
4. **The build matrix is the single source of truth.** `scripts/build-platforms.ts`
   ...
   `sikong`'s `optionalDependencies` drift from the matrix.
5. **Release order is platforms-first.** `scripts/release.ts` builds, publishes
   every `sikong-<key>`, then publishes the launcher last so its optional
   dependencies already resolve on the registry.

## Consequences

- `npm install sikong` works on all seven targets, downloading only the host's
...
- `bin/sikong` — launcher with `platformKey()` + `detectLibc()`.
