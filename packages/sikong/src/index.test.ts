...
import { SIKONG_VERSION, DEVELOPMENT_WORKFLOW, GENERAL_WORKFLOW, validateWorkflow } from "./index";

// Proves the monorepo wiring: sikong resolves and drives agent-loop
// across the package boundary (workspace:* symlink), and exposes its own kernel.
describe("sikong ⇄ agent-loop wiring", () => {
  ...
  test("exports the workflow kernel + a version marker", () => {
    expect(SIKONG_VERSION).toBe("0.1.6");
