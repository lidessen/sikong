...
describe("workspace (CLI wiring)", () => {
  ...
  test("workspace dir resolution defaults to SIKONG_HOME and preserves explicit legacy overrides", () => {
    expect(resolveWorkspaceDir({ dirFlag: "/tmp/explicit", env: { SIKONG_DIR: "/tmp/legacy", SIKONG_HOME: "/tmp/home" } }).dir).toBe(
      "/tmp/explicit",
    );
    expect(resolveWorkspaceDir({ env: { SIKONG_DIR: "/tmp/legacy", SIKONG_HOME: "/tmp/home" } }).dir).toBe(
      "/tmp/legacy",
    );
    expect(resolveWorkspaceDir({ env: { SIKONG_HOME: "/tmp/home" } }).dir).toBe("/tmp/home");
    expect(resolveWorkspaceDir({ env: {} }).dir).toMatch(/\.sikong$/);
  });
