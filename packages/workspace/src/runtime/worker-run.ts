...
export function buildStageWorkerPrompt(
  ...
  return [
    ...
    "",
    "Your responsibility is to complete only this work unit inside the current stage round and return concrete evidence of what changed, what was verified, and what remains. The Task Lead plans rounds, and Reviewers decide whether the stage or task is acceptable.",
    "",
    "Scope contract:",
    ...
    ...workUnit.instructions.map((item) => `- ${item}`),
    "",
    "Expected deliverables:",
    ...workUnit.deliverables.map((item) => `- ${item}`),
    "",
    "Out of scope:",
    ...workUnit.outOfScope.map((item) => `- ${item}`),
