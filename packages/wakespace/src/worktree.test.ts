...
describe("worktree isolation (ADR 0010)", () => {
  ...
  test("ensure → edit → release(done) commits to the branch and removes the worktree", async () => {
    ...
    try {
      ...
      expect((await git(wt, ["rev-parse", "--abbrev-ref", "HEAD"])).stdout.trim()).toBe("wakespace/task_a");
      ...
      expect((await git(root, ["log", "--oneline", "wakespace/task_a"])).stdout).toContain("wakespace subtask task_a");
      expect((await git(root, ["ls-tree", "--name-only", "wakespace/task_a"])).stdout).toContain("greet.ts");
    } finally {
  ...
  test("release(cancelled) drops the worktree and its branch", async () => {
    ...
    try {
      ...
      expect((await git(root, ["branch", "--list", "wakespace/task_c"])).stdout.trim()).toBe("");
    } finally {
      await cleanup(root, dir);
    }
  });
