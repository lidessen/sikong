...
/**
 * Git-worktree isolation for wakespace (ADR 0010). This is the ONLY place that
 * knows about git: it backs the worker-boundary `isolateWorkspace`/`releaseWorkspace`
 * hooks. Worktrees live UNDER the wakespace home dir (`<dir>/worktrees/<taskId>`),
 ...
 * `wakespace/<taskId>` for the lead to integrate.
 */
 ...
 * reuses the worktree across the task's wakes. Creates branch `wakespace/<taskId>`
...
export async function releaseWorktree(
  ...
  if (status === "done" && (await exists(wt))) {
    ...
    if (!clean) await gitQuiet(wt, ["commit", "--no-verify", "-m", `wakespace subtask ${taskId}`]);
  }
