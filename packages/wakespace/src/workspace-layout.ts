...
export const WAKESPACE_HOME_ENV = "WAKESPACE_HOME";
export const WAKESPACE_DIR_ENV = "WAKESPACE_DIR";

export type WorkspaceDirSource = "flag" | "WAKESPACE_DIR" | "WAKESPACE_HOME" | "default";
...
export function defaultWorkspaceHome(env: NodeJS.ProcessEnv = process.env): string {
  const home = env[WAKESPACE_HOME_ENV]?.trim();
  return home || join(homedir(), ".wakespace");
}
...
export function resolveWorkspaceDir(opts: { dirFlag?: string; env?: NodeJS.ProcessEnv } = {}): ResolvedWorkspaceDir {
  ...
  const legacy = env[WAKESPACE_DIR_ENV]?.trim();
  if (legacy) return { dir: legacy, source: "WAKESPACE_DIR" };
  const home = env[WAKESPACE_HOME_ENV]?.trim();
  if (home) return { dir: home, source: "WAKESPACE_HOME" };
