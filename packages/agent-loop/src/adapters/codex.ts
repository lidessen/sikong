...
export class CodexAdapter implements BackendAdapter {
  ...
  start(req: ResolvedRequest): BackendRun {
    ...
    const finish = (err?: Error) => {
      ...
      if (!sawUsage) {
        const turnText = req.system ? `${req.system}\n\n${req.prompt}` : req.prompt;
    ...
    const run = async () => {
      ...
      const threadResp = (await client.request(threadMethod, {
        ...
        sandbox: this.opts.sandbox ?? (this.opts.fullAuto ? "workspace-write" : undefined),
  ...
  private async handleCodexRequest(
    ...
    switch (request.method) {
      ...
      case "item/permissions/requestApproval": {
        ...
        if (params.permissions?.fileSystem) permissions.fileSystem = params.permissions.fileSystem;
      ...
      case "item/tool/call": {
        ...
        if (hooksBridge) {
          ...
          if (decision.action === "deny") {
            return {
              contentItems: [
                { type: "inputText", text: decision.reason ?? "Tool call denied by policy." },
              ],
              success: false,
            };
          }
