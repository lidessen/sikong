...
describe("createEscalationOnToolUse", () => {
  ...
  test("approves allow-listed build/test commands on a bash tool", async () => {
    expect(await hook({ name: "Bash", args: { command: "swift build" } })).toEqual({
      action: "approve",
    });
    expect(await hook({ name: "Bash", args: { command: "bun run test" } })).toEqual({
      action: "approve",
    });
    expect(await hook({ name: "Bash", args: { command: "git status" } })).toEqual({
      action: "approve",
    });
  });
  ...
  test("defers deny-classified commands to the runtime posture (continue)", async () => {
    expect(await hook({ name: "Bash", args: { command: "rm -rf build" } })).toEqual({
      action: "continue",
    });
    expect(await hook({ name: "Bash", args: { command: "git push --force" } })).toEqual({
      action: "continue",
    });
  });
  ...
  test("never affects non-bash tools", async () => {
    ...
    expect(await hook({ name: "Edit", args: { command: "swift build" } })).toEqual({
      action: "continue",
    });
  });
  ...
  test("is a no-op when escalation is disabled", async () => {
    ...
    expect(await off({ name: "Bash", args: { command: "swift build" } })).toEqual({
      action: "continue",
    });
  });
});
