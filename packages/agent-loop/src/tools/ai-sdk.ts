...
export interface AiSdkToolOptions {
  /** Local workspace root. File and shell tools are scoped to this directory. */
  cwd: string;
...
const rgSchema = z.object({
  ...
  path: z
    .string()
    .optional()
    .describe("File or directory to search, relative to the workspace root."),
  glob: z
    .union([z.string(), z.array(z.string())])
    .optional()
    .describe("One or more rg --glob filters."),
  max_results: z
    .number()
    .int()
    .min(1)
...
const webFetchSchema = z.object({
  ...
  max_bytes: z
    ...
    .max(500_000)
    .optional()
    .describe("Maximum response bytes to read."),
  raw: z
    .boolean()
    .optional()
    .describe("Return raw response text instead of extracting readable text from HTML."),
});
...
const insertInFileSchema = z.object({
  path: z.string().min(1).describe("Existing file to edit, relative to the workspace root."),
  text: z.string().min(1).describe("Line block to insert."),
  position: z
    .enum(["before", "after", "end"])
    .optional()
    .describe(
      "Where to insert the text relative to `line`. Defaults to after when `line` is set, otherwise end.",
    ),
  line: z
    .number()
    .int()
  ...
  expected_line: z
    ...
    .optional()
    .describe(
      "Optional exact content of the anchor line, without line number or newline. If it differs, no file is written.",
    ),
});

const viewFileSchema = z.object({
  path: z.string().min(1).describe("File to inspect, relative to the workspace root."),
  start_line: z
    .number()
    .int()
    .min(1)
    .optional()
    .describe("1-based first line to show. Defaults to 1."),
  max_lines: z
    .number()
    .int()
    .min(1)
    .max(500)
    .optional()
    .describe("Maximum lines to return. Defaults to 120."),
});

export async function createAiSdkTools(options: AiSdkToolOptions): Promise<ToolSet> {
  ...
  const { tools: bashTools } = await createBashTool({
    ...
    extraInstructions:
      "This sandbox maps / to the workspace root. File reads and writes are constrained to that root; symlink traversal is rejected by the filesystem.",
  });
 ...
 */
function wrapBashWithEscalation(tool: ToolDefinition, opts: BashEscalationOpts): ToolDefinition {
  return {
    ...tool,
    execute: async (rawArgs, ctx) => {
    ...
    execute: async (rawArgs) => {
      ...
      if (position !== "end") {
        ...
        if (line > lines.length) {
          return {
            error: `Line ${line} is outside "${pathResult.rgPath}" (${lines.length} lines).`,
          };
        }
      ...
      const after =
        afterLines.join(newline) + (hasFinalNewline(before) || position === "end" ? newline : "");
      try {
        await writeFile(filePath, after, "utf8");
      } catch (err) {
        return { error: `Could not write "${pathResult.rgPath}": ${(err as Error).message}` };
...
function createWebFetchTool(options: AiSdkToolOptions): ToolDefinition {
  return defineTool({
    description:
...
function createWebSearchTool(options: AiSdkToolOptions): ToolDefinition {
  return defineTool({
    description:
...
async function resolveProjectPath(
  ...
  if (rel === ".." || rel.startsWith(`..${separatorFor(rel)}`) || isAbsolute(rel)) {
    return { ok: false, error: `Path "${inputPath}" is outside the workspace root.` };
  }
  try {
    ...
    if (
      realRel === ".." ||
      realRel.startsWith(`..${separatorFor(realRel)}`) ||
      isAbsolute(realRel)
    ) {
      return { ok: false, error: `Path "${inputPath}" resolves outside the workspace root.` };
    }
  } catch {
...
async function validatePublicHttpUrl(
  ...
  if (addresses.length === 0)
    return { ok: false, error: "URL host did not resolve to an address." };
...
function extractHtmlTitle(html: string): string | undefined {
  ...
  return (
    decodeHtmlEntities(collapseWhitespace(stripTags(html.slice(start + 1, end)))).trim() ||
    undefined
  );
}
