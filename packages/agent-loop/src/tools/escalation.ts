...
const TOOLCHAIN_COMMANDS = new Set([
  "swift", "go", "bun", "npm", "npx", "yarn", "pnpm",
  "cargo", "rustc", "make", "cmake", "gradle", "mvn",
  "dotnet", "xcodebuild", "bazel", "ninja",
  "tsc", "vitest", "jest", "mocha", "ava",
  "eslint", "prettier", "biome", "shellcheck", "typos",
  "rg", "grep",
]);
...
export async function runOnHost(
  ...
  try {
    ...
    return {
      exitCode,
      stdout,
      stderr: nodeErr.signal
        ? `${stderr}\n[killed: signal ${nodeErr.signal}]`
        : stderr,
