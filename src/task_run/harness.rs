    ...
    match context.operation {
        NodeOperation::Specify => with_operation_context(
            ...
            operation_prompt! {
                ...
                "Size Reading" {
                    "Pick the smallest safe size by coordination cost while preserving the user's full current intent, judging evidence boundaries before judging the final artifact shape. tiny is a direct answer from current context. small is one local problem with one obvious verification path. medium is one coherent responsibility or change package whose evidence cannot usefully be accepted in separate parts — all pieces serve the same behavioral fix. large means planning reduces risk: ordered phases, shared evidence feeding later work, multiple independently accepted deliverables, separate responsibilities that deserve separate verification, or a single final proposal built from several independently inspectable evidence surfaces. x_large means multiple independent targets or product delivery across several major surfaces.\n\nDo not treat 'same theme', 'one report', 'cross-surface comparison', or 'one worker can hold the context' as enough reason to keep the work medium. Cross-surface comparison is a Combine responsibility after child surfaces have produced evidence; it is not a reason to make evidence collection atomic. A repository audit across several top-level subsystems, packages, runtime boundaries, eval surfaces, or doc families should enter Plan unless the user only asked for a plan document. Local change means one behavior or one code path with supporting prompt/test/eval/doc updates — separate packages, subsystems, runtime boundaries, or doc families named as audit targets are independent evidence surfaces, not nearby parts of one local change. Keep the work medium only when the evidence is useful only when inspected together from the start.\n\nDo not count surfaces mechanically. When unsure, ask whether splitting would make the result more reliable or only add coordination overhead; if splitting only adds overhead, keep the next unit medium."
                }
                "Boundary" {
    ...
    pub fn decode_result(
        ...
        match result.terminal_call {
            ...
            None => Err(AgentRunDecodeError {
                message: agent_run_failure_message(&result).unwrap_or_else(|| {
                    format!(
...
fn agent_run_failure_message(result: &AgentRunResponse) -> Option<String> {
    result.events.iter().find_map(|event| {
        let object = event.as_object()?;
        if object.get("event")?.as_str()? != "agent_run_failure" {
            return None;
        }
        let class = object
            .get("class")
        ...
        let message = object
            .get("message")
            .and_then(Value::as_str)
            .unwrap_or(result.report.as_str());
        Some(format!(
            "agent run failed ({class}): {message}; scheduler did not produce a terminal tool"
        ))
    })
}
