# ADR 0008: Sikong Owns Staffing; the Client Only States, Accepts, Supervises
...
sikong (the controller) never teaches it how to work. This ADR fixes the
...
plant's internals and stays out of sikong. "Who is assigned to a task" is
...
- **Sikong** — the controller: intake/research, plan, create & decompose tasks,
...
1. **Provision vs assign.** The operator provisions the workforce *once* — by
   ...
   `claude`). Per-task *assignment* is sikong's job, every task. sikong
   cannot conjure credentials, so provisioning is the only external seam.
2. **Auto-discovered roster.** When no workers are explicitly registered, sikong
...
3. **Capability-matched assignment.** A worker carries optional `roles` (capability
   ...
   needs). At hire time sikong prefers a worker whose roles include the
...
- The client interacts with sikong as a company: requirement in, result out.
