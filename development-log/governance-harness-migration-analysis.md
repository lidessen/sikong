...
The action (rejecting an Accept+hard_violations verdict) is the same. The diagnostic value is preserved because the reason still explains *why* it was rejected. Downstream judge logic (eval scripts) that pattern-matches on gate names would need to switch to matching on the reason text or the verdict structure (`Accept` verdict with non-empty `hard_violations` always produces `Reject`).
